...
# --- SIDEBAR: BLINDAJE CONTRA KEYERROR ---
with st.sidebar:
    st.header("📊 Historial")
    if os.path.exists("parlay_history.csv"):
        try:
            ...
            if not hist.empty:
                # Normalizamos nombres de columnas a minúsculas para evitar KeyError
...
if archivo:
    ...
    if games:
        with st.expander("🏟️ Verificación"):
        ...
        if parlay:
            ...
            # CORRECCIÓN DE SYNTAX ERROR (Línea 69-70)
            for p in parlay:
                st.markdown(f"""
