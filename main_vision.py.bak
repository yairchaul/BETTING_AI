...
from modules.parlay_reasoning_engine import ParlayReasoningEngine  # NUEVO: Motor de Regla 6
...
def select_parlay_picks(additional_markets, probs_1x2, home, away):
    ...
    if over_1_5_1t > 0.50:  # Cambiado de 60% a 50%
        picks.append({
            'market': 'Over 1.5 1T',
            'prob': over_1_5_1t,
            'type': 'over_1t',
    ...
    if btts > 0.50:  # Cambiado de 60% a 50%
        picks.append({
            'market': 'BTTS Sí',
            'prob': btts,
            'type': 'btts',
...
def init_components():
    return {
        ...
        'reasoning_engine': ParlayReasoningEngine(umbral_principal=0.50)  # NUEVO: Motor de Regla 6
    }
            ...
            if len(all_parlay_picks) >= 2:
                ...
                for pick in picks_finales:
                    if pick.get('tipo') == 'combinado':
                        ...
                        for p in pick['picks']:
                            st.markdown(f"   - {p['market']} ({p['prob']:.1%})")
                            total_prob *= p['prob']
                            total_odds *= p['odds']
                    else:
                        ...
                        if st.button("📝 Registrar este parlay"):
                            ...
                            for pick in picks_finales:
                                picks_guardar.append({
                                    ...
                                    'market': pick.get('market', 'COMBINADO'),
