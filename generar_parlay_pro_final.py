...
for p in partidos:
    ...
    print(f'   Stats {local}: GF={local_stats["avg_goals_scored"]:.2f}, GA={local_stats["avg_goals_conceded"]:.2f} ({local_stats["source"]})')
    print(f'   Stats {visitante}: GF={visit_stats["avg_goals_scored"]:.2f}, GA={visit_stats["avg_goals_conceded"]:.2f} ({visit_stats["source"]})')
    
    # Calcular fuerzas
    home_attack = local_stats['avg_goals_scored']
    home_defense = local_stats['avg_goals_conceded']
    away_attack = visit_stats['avg_goals_scored']
    away_defense = visit_stats['avg_goals_conceded']
    ...
    # Crear picks con EV real
    picks_candidatos = [
        # Mercados 1X2
        {'name': 'Gana Local', 'prob': elo_probs['home'], 'odds': odds_local, 'category': '1X2'},
        {'name': 'Empate', 'prob': elo_probs['draw'], 'odds': odds_draw, 'category': '1X2'},
        {'name': 'Gana Visitante', 'prob': elo_probs['away'], 'odds': odds_visit, 'category': '1X2'},
        
        # Mercados BTTS
        {'name': 'BTTS - Sí', 'prob': btts_prob, 'odds': 1/btts_prob * 0.95 if btts_prob > 0 else 0, 'category': 'BTTS'},
        
        # Mercados Totales
        {'name': 'Over 1.5', 'prob': over1_5, 'odds': 1/over1_5 * 0.95 if over1_5 > 0 else 0, 'category': 'Totales'},
        {'name': 'Over 2.5', 'prob': over2_5, 'odds': 1/over2_5 * 0.95 if over2_5 > 0 else 0, 'category': 'Totales'},
    ]
    
    # Calcular EV para cada pick
    for pick in picks_candidatos:
        if pick['odds'] > 0:
            pick['ev'] = (pick['prob'] * pick['odds']) - 1
            pick['match'] = f'{local} vs {visitante}'
            
            if pick['ev'] > 0.05:  # Solo EV positivo significativo
                all_picks.append(pick)
                print(f'    {pick["name"]}: Prob {pick["prob"]:.1%}, Odds {pick["odds"]:.2f}, EV {pick["ev"]:.1%}')

# Optimizar parlay
print('\n OPTIMIZANDO PARLAY...')
best_parlay = optimizer.find_best_parlay(all_picks, max_size=3, min_ev=0.10)
...
if best_parlay:
    ...
    print(' No se encontró parlay con EV positivo suficiente')
